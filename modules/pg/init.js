...
module.exports = async function init(db) {
    ...
    const acount = await db.address.count(); 
    if(acount === 0){
    ...
    const m_count = await db.assets.count(0); 
    if(m_count === 0){
        const main_info = await db.assets.create({
            main_title: "Talaba bo‘lishingizga albatta ishonamiz",
            main_motto: "Shaxmatda yo’lga chiqqan piyoda, bir kun kelib albatta vazir bo’ladi."
        })
    }
}
